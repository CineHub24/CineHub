...
export class EmailService {
  ...
  async sendNewFilmEmail(filmId: number, recipientEmail: string): Promise<void> {
    const filmToInform:Film[] = await db
      .select()
      .from(film)
      .where(eq(film.id, filmId)).limit(1);
    const emailContent = `
    <img src=${filmToInform[0].backdrop}>
    <p>Hallo Filmfan!</p>

<p>Wir freuen uns, dir ${filmToInform[0].title}, den neuesten Film von dem renommierten Regisseur ${filmToInform[0].director}, präsentieren zu können. Tauche ein in eine fesselnde ${filmToInform[0].genres}-Geschichte, die dich garantiert begeistern wird.</p>

<p>${filmToInform[0].description}</p>

<p> Interressiert? Dann Erlebe den Film jetzt auf unserer großen Leinwand und lass dich mitreißen.

<p>Tickets gibt's hier: <a href="http://localhost:5173/film/${filmToInform[0].id}">Jetzt Tickets sichern</a></p>

<p>Bis bald im Kino!</p>

<p>Dein CineHub-Team</p>
    `;
    try {
      await this.transporter.sendMail({
        from: `"CineHub Newsletter" <${this.gmailUser}>`,
        to: recipientEmail,
        subject: `${filmToInform[0].title}: ${filmToInform[0].director} entführt dich in eine ${filmToInform[0].genres}-Welt!`,
        html: emailContent
      });
    } catch (error) {
      console.error('Fehler beim Versenden der E-Mail:', error);
