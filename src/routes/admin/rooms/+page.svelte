<script lang="ts">
  // Import the `cinemaHalls` from the load function
  let data = $props();

  let cinemaHalls = $state(data.data.cinemaHalls);
</script>

<div class="container">
  <div class="header">
    <a href="/admin/rooms/create" class="add-button">add room</a>
    <a href="/admin/seat-category" class="add-button">seat categrys</a>
  </div>
  ...
  {#if cinemaHalls.length > 0}
    <ul class="hall-list">
      {#each cinemaHalls as hall}
        <li class="hall-item">
          <div class="hall-details">
            <span><strong>Name:</strong> {hall.name}</span>
