...
describe('Profile Page', () => {
	...
	describe('changePassword action', () => {
		...
		it('should fail if new passwords do not match', async () => {
			...
			const result = await actions.changePassword(event as any);
			expect(result).toEqual(
				fail(400, { error: 'Die neuen Passwörter stimmen nicht überein', mismatch: true })
			);
		});
		...
		it('should fail if new password is invalid', async () => {
			...
			expect(result).toEqual(
				fail(400, {
					error:
						'The new password must contain uppercase & lowercase letters, numbers, and special characters, and be between 8 and 255 characters long.',
					invalid: true
				})
			);
		});
