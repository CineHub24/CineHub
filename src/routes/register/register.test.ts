...
function createMockRequestEvent(
	formData = {}
): RequestEvent<Partial<Record<string, string>>, '/register'> {
	return {
		request: {
...
function createMockServerLoadEvent(): ServerLoadEvent<
...
describe('Register Action', () => {
	...
	it('should handle database errors', async () => {
		...
		expect(result).toEqual(
			fail(500, { message: expect.stringContaining('An error has occurred') })
		);
	});
});
