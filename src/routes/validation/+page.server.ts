...
import { cinemaHall, seat, showing, ticket, type Showing, type Ticket } from '$lib/server/db/schema';
...
export const actions = {
    validate: async ({ request }) => {
        try {
            ...
            const foundTicket = await db.select().from(ticket).innerJoin(seat,eq(seat.id, ticket.seatId)).where(eq(ticket.token, ticketToken));
            const show  = await db.select().from(showing).innerJoin(cinemaHall,eq(cinemaHall.id, showing.hallid)).where(eq(showing.id, foundTicket[0]?.showingId ?? 0));
            ...
            return {
                success: true,
                message: `Ticket gültig - Saal ${show[0].CinemaHall.name}, Sitz ${foundTicket[0].seat.row}${foundTicket[0].seat.seatNumber}`,
